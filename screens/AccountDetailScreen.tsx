...
export default function AccountDetailScreen({ account, onBack }: AccountDetailScreenProps) {
  ...
  const fetchDetail = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${SERVER_URL}/api/v1/account/${encodeURIComponent(accountNo)}`,
        {
          method: "GET",
          headers: {
      ...
      const mappedTransactions = accountDetailResponse.list.map(
        (transaction: TransactionDetailDTO, index: number) =>
  ...
  return (
    <View style={styles.container}>
      ...
      >
        {/* 계좌 정보 카드 */}
        <AccountCard account={AccountMapper.fromLegacyAccount(account)} onPress={() => {}} />
        ...
        <View style={styles.transactionsContainer}>
          <Text style={styles.transactionsTitle}>거래내역 ({filteredTransactions.length}건)</Text>
