...
export function useAddPlanScreen() {
  ...
  const uploadImages = async (): Promise<string[]> => {
    ...
    try {
      ...
      console.log(`총 ${formData.files.length}장의 이미지를 업로드합니다.`);
      ...
      console.log("추출된 이미지 URL들:", imageUrls);
  ...
  const createAddPlanRequest = async (): Promise<AddPlanRequestEntity> => {
    ...
    const requestEntity: AddPlanRequestEntity = {
      ...
      savingsAmount:
        formData.saveOption && formData.savingAmount ? parseInt(formData.savingAmount) : null,
      depositAccountNo:
        formData.saveOption && formData.depositAccount
          ? formData.depositAccount.accountNumber
          : undefined,
      withdrawalAccountNo:
        formData.saveOption && formData.withdrawalAccount
          ? formData.withdrawalAccount.accountNumber
          : undefined,
  ...
  const handleAddPlan = async () => {
    ...
    console.log(
      "일정 유형:",
      formData.eventType === "group" ? "GROUP (단체 일정)" : "PERSONAL (개인 일정)"
    );
    ...
    if (formData.saveOption) {
      console.log(
        ...
        formData.withdrawalAccount
          ? `${formData.withdrawalAccount.bankName} - ${formData.withdrawalAccount.accountNumber}`
          : "선택되지 않음"
      );
      console.log(
        "입금계좌:",
        formData.depositAccount
          ? `${formData.depositAccount.bankName} - ${formData.depositAccount.accountNumber}`
          : "선택되지 않음"
      );
      console.log(
        "저축금액:",
        formData.savingAmount ? `${Number(formData.savingAmount).toLocaleString()}원` : "0원"
      );
    }
    console.log("");
    ...
    console.log(
      "FormData 객체가 생성되었습니다. (React Native에서는 entries() 메서드를 지원하지 않습니다)"
    );
    console.log("");
    ...
    try {
      ...
      const response = await fetch(`${SERVER_URL}/api/v1/plans`, {
        method: "POST",
